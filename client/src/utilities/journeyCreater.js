...
const failureReason = (failure) => {
  // console.log('Failure reason --', failure);
...
const positionRemark = (startPosi, endPosi, driver) => {
  if (startPosi >= endPosi) {
    ...
    return `This was not the result expected by ${driver}. ${driver} ended at position ${endPosi} despite starting at ${startPosi}`;
  }
};
export const createSeasonJourney = (raceDetail, idx) => {
  ...
  if (raceDetail?.Results[0].status !== 'Finished') {
    ...
    } was not able to finish this race due to ${failureReason(
      raceDetail.Results[0].status
    )}. No points acquired in this race. `;
  } else {
    type = 'success';
    remark += `${
      raceDetail.Results[0].Driver.familyName
      ...
      raceDetail.Results[0].Driver.familyName
    )}. ${raceDetail.Results[0].laps} laps were done in this race. ${
      raceDetail.Results[0].FastestLap &&
      `The fastest lap occured on ${raceDetail.Results[0].FastestLap.lap} in time period of ${raceDetail.Results[0].FastestLap.Time.time} at average speed of ${raceDetail.Results[0].FastestLap.AverageSpeed.speed} ${raceDetail.Results[0].FastestLap.AverageSpeed.units}`
    } .${raceDetail.Results[0].points} points acquired by ${
      raceDetail.Results[0].Driver.familyName
    } in this race.`;
  }
  return [type, remark];
